...
for m in range(l):
    ...
    if par[goodness_parameter][m] <= treshold:
        ...
        if os.path.exists('magnitude_plots/output_plots/'+ par['name'][m]  ) == False:
            os.mkdir('magnitude_plots/output_plots/' + par['name'][m]  )
        ...
        plt.savefig('magnitude_plots/output_plots/' + par['name'][m] + '/' + par['name'][m] + 'allbands')
        #plt.show()
        plt.close()
